...
describe("Redux posts reducer testing", () => {
  describe("Action types", () => {
    test("GET_DUMMY_POSTS", () => {
      expect(posts(defaultPosts, { type: "GET_DUMMY_POSTS" })).toEqual({
        status:  "success",
        search:  [
          {
            id:           "569bfcdc-fad1-4563-ae57-8585831db596",
    ...
    test("TOGGLE_DISPLAY", () => {
      /**
       * Toggle from default to favs
       */
      let store = posts(defaultPosts, { type: "TOGGLE_DISPLAY" });
      ...
      /**
       * Toggle from favs to search
       */
      store = posts(store, { type: "TOGGLE_DISPLAY" });
      expect(store).toEqual({
        status:  "success",
        search:  [],
        favs:    [],
    ...
    test("FETCH_POSTS_ERROR", () => {
      expect(posts(defaultPosts, { type: "FETCH_POSTS_ERROR" })).toEqual({
        status:  "error",
        search:  [],
        favs:    [],
        filter:  "",
    ...
    test("FETCH_POSTS_SUCCESS", () => {
      expect(posts(defaultPosts, { type: "FETCH_POSTS_SUCCESS", data: [dummyPost] })).toEqual({
        status:  "success",
        search:  [dummyPost],
        favs:    [],
        filter:  "",
    ...
    test("FETCH_POSTS_404", () => {
      expect(posts(defaultPosts, { type: "FETCH_POSTS_404" })).toEqual({
        status:  "not_found",
        search:  [],
        favs:    [],
        filter:  "",
    ...
    test("TO_FAVORITE", () => {
       ...
       */
      store = posts(store, { type: "TO_FAVORITE", data: dummyPost });
      expect(store).toEqual({
        status:  "success",
        search:  [],
        favs:    [],
    ...
    test("FETCH_FAVORITES_ERROR", () => {
      expect(posts(defaultPosts, { type: "FETCH_FAVORITES_ERROR" })).toEqual({
        status:  "error",
        search:  [],
        favs:    [],
        filter:  "",
    ...
    test("FETCH_FAVORITES_SUCCESS", () => {
      expect(posts(defaultPosts, { type: "FETCH_FAVORITES_SUCCESS", data: [dummyPost] })).toEqual({
        status:  "success",
        search:  [],
        favs:    [dummyPost],
        filter:  "",
